...
"""
    ...
    def export_environment(self) -> 'ElasticBeanstalkBuilder':
        ...
        os.environ['S3_BUCKET_NAME'] = self.resources.get('s3_bucket_name', self.settings.s3_bucket_name)
        os.environ['SQS_QUEUE_URL'] = self.resources.get('sqs_queue_url', '')
        os.environ['SQS_QUEUE_NAME'] = self.settings.sqs_queue_name
        os.environ['EB_APP_NAME'] = self.resources.get('eb_app_name', EB_APP_NAME)
        os.environ['EB_ENV_NAME'] = self.resources.get('eb_env_name', EB_ENV_NAME)
        
        if self.mode == "aws-mock":
            os.environ['AWS_ENDPOINT_URL'] = self.strategy.endpoint_url
            os.environ['AWS_ACCESS_KEY_ID'] = self.settings.aws_access_key_id or 'mock'
            os.environ['AWS_SECRET_ACCESS_KEY'] = self.settings.aws_secret_access_key or 'mock'
